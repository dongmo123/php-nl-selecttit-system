...
<html>
	...
	<body>
		<div id="container">
			<div id="header">
				...
				<ul class="nav1">
					...
					<li><a href="./home/main.php" target="iframe">选题流程</a></li>
				...
				<span style="position:absolute;right:20px;top:40px;text-align:right;">
					<?php
						...
						*/
						...
						$sqlnum="update online set id=null,student='{$total_online}'";
						mysqli_query($link,$sqlnum);

						$sqltnum="select teacher,admin from online";
						$resulttnum=mysqli_query($link,$sqltnum);
						$rowtnum=mysqli_fetch_array($resulttnum);
						echo '当前在线：学生'.$total_online.'人<br>教师'.($rowtnum['teacher']).'人<br>管理员'.($rowtnum['admin']).'人';
					?>
				</span>
			</div>
			<div id="content" name="availHeight">
				<div class="left" id="left">
					<div class="left-top">
						...
						<div>
							<div>学号：<?php echo $_SESSION['stunum'];?></div>
							<div>姓名：<?php echo $_SESSION['stuname'];?></div>
						</div>
					</div>
					<ul class="sdmenu" id="mymenu">
						<li>
							...
							<div class="off" id="one">
								...
								<a href="home/mytit.php" target="iframe">我的选题</a>
							</div>
						</li>
						<li>
							<span class="on">开始选题</span>
							<div class="off" id="two">
								<a href="home/alltit.php" target="iframe">选题列表</a>
								<a href="home/allstuinf.php" target="iframe">所有学生选题记录</a>
							</div>
						</li>
						<li>
							<span class="on">留言建议</span>
							<div class="off" id="four">
		...
		<div id="footer">
				<div>如果显示异常,先尝试调成浏览器兼容模式,若不行请更新或更换浏览器!最佳浏览器：Chrome57.0及其以上版本。
				</div>
		</div>
	</body>
</html>
...
<script type="text/javascript">
    function dodel(){
        if(confirm ("确认要注销或切换用户吗?")){
            window.location="../public/usersaction.php?element=thanksstu";
        }
    }
</script>