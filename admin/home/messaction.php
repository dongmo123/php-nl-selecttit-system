...
switch($_GET['type']){
    case "insert":  //执行添加操作
        ...
        $pass=$_POST['pass'];
        //4.1.3拼装SQL语句并发送执行
        $sql = "insert into message (ID,username,usertype,content,time,targetuser,pass) values(null,'{$username}','{$usertype}','{$content}',now(),'{$targetuser}','{$pass}')";
    ...
    case "deleteall":  //执行删除操作
        ...
        if(!empty($_POST['username'])){
            //搜索专业
            $wherelist[] = "username like '%{$_POST['username']}%'";
        }
        if(!empty($_POST['usertype'])){
            //搜索班级
            $wherelist[] = "usertype like '%{$_POST['usertype']}%'";
        }
        //判断并拼装搜索条件
        $where = "";
        ...
        //通过影响的行数来判断成功与否
        if(mysqli_affected_rows($link)>0){
            echo "<script type='text/javascript'>window.onload=function(){update('批量删除留言成功！！！');history.go(-1);}</script>";
        }else{
            echo "<script type='text/javascript'>window.onload=function(){update('批量删除留言失败~~~可能没有此数据');history.go(-1);}</script>";
        }
        break;
}
